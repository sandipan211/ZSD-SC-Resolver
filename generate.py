...
def load_seen_att(opt):
    ...
    #print(f'seen_class_id: {get_seen_class_ids(opt.dataset, split=opt.classes_split)}')
...
def load_unseen_att(opt):
    attribute, labels = load_all_att(opt)
    #print(f'unseen_class_id: {get_unseen_class_ids(opt.dataset, split=opt.classes_split)}')
