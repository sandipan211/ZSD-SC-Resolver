...
def load_rpn_conf_cls_weights(model):
    wieghts_file_path = '/workspace/arijit_ug/sushil/zsd/checkpoints/coco_65_15_wgan_modeSeek_seen_cycSeenUnseen_triplet_varMargin_try21/classifier_best.pth'
    if hasattr(model, 'module'):
        own_state = model.module.state_dict()
    else:
        own_state = model.state_dict()
    
    checkpoint = torch.load(wieghts_file_path, map_location='cpu')
    if isinstance(checkpoint, OrderedDict):
        state_dict = checkpoint
    elif isinstance(checkpoint, dict) and 'state_dict' in checkpoint:
        state_dict = checkpoint['state_dict']

    own_state['rpn_conf_cls.bias'] = state_dict['fc1.bias']
    own_state['rpn_conf_cls.weight'] = state_dict['fc1.weight']
...
def main():
    ...
    # mmlab
    # load_rpn_conf_cls_weights(model)
