...
class TrainGAN():
    def __init__(self, opt, attributes, unseenAtt, unseenLabels,seen_attributes,seen_attr_labels, seen_feats_mean, gen_type='FG'):
        ...
        ### Wu_labels : labels for unsenn class varies from 0 to 14
        ...
        self.reg_criterion= nn.MSELoss(reduction='sum') #loss function for cyc loss squared l2 norm
        print(f"regressor_lambda: {self.opt.regressor_lamda}")
        ### setting the requires_grad to false
        for p in self.classifier.parameters():
            p.requires_grad = False
        
        #setting the requires_grad to false for regressor
        ...
        print('''
              ...
              wgan+ modeSeek+triplet with seen and unseen +seen + cyclic on seen and unseen
              
                triplet unseen generating bsize//16
                permutation used
                seed added for reproducibility of code
    ...
    def trainEpoch(self):
        ...
        for i in range(0, self.ntrain, self.opt.batch_size):
            ...
            # ##generating featurs and corresponding label for unseen class
            # #############for unseen part
            ...
            true_unseen_semantic = self.Wu[fake_unseen_lab] #as calculating loss between true semantic embedding and the predicted semantic
            #reg_criterion= nn.L1Loss() #loss function for cyc loss
            ...
            fake_generated_seen = self.netG(noise3v, input_attv) #generated seen_image conditioned on semantics
            ...
            errG= -G_cost + c_errG + loss_lz   + cal_triplet_loss + cyc_err 
            ...
            tripet_loss : {cal_triplet_loss.item()}  triplet_unseen_loss: {self.opt.triplet_lamda*t_loss_unseen.data.item()}  cyc_loss.item: {cyc_err.data.item():.4f}")
